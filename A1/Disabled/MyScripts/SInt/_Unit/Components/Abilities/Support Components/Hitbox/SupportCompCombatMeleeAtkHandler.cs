...
public class SupportCompCombatMeleeAtkHandler : MonoBehaviour,ISupportComp
{
    ...
    [Header("Spawn particle FX based on number of Contact Points Casted")]
    public int contactPointHitboxCount = 8;
    ...
    private void Start()
    {
        ...
        _hittableRigidHandler.InitializePool(contactPointHitboxCount);
        //Ref: https://youtu.be/OMau8bhENp4?list=PLPYO_4PaVtLU-cxSTYaIGpK3PbBMDbldU&t=710
    }
    private void LateUpdate()
    {
        //if (isDebugTrail)
        if (_ownerUnitRefs._anim.GetBool(nameof(ECharAnimParam.isInstigateDamageOn)))
        {
            CheckTrail();
        }
    }
    ...
    public void OnAttack(int attackType)
    {
        ...
        OnAttackByType(attackType);
    }

    public void OnAttack_SetAnimParam(int attackType)
    {
        if (_ownerUnitRefs._anim.GetBool(nameof(ECharAnimParam.CanAttack)))
        {
            attackId++;
            _ownerUnitRefs._anim.SetTrigger(nameof(ECharAnimParam.Attack));
            _ownerUnitRefs._anim.SetInteger(nameof(ECharAnimParam.AttackType), attackType);
            _hittableRigidHandler.ClearCollisionPointList();
        }
    }
    public void OnAttackByType(int attackType)
    {
        if (_ownerUnitRefs._anim.GetBool(nameof(ECharAnimParam.isEvading)))
        {
            return;
        }
